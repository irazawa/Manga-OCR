# Manga OCR & Typeset Tool v3.7
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v3.7 ---
# 1. Zoom Tetap (Persistent Zoom): Tingkat zoom tidak akan direset saat
#    berganti gambar, memberikan alur kerja yang lebih konsisten.
# 2. Fungsi Redo: Menambahkan fungsionalitas Redo (Ctrl+Y) untuk mengembalikan
#    tindakan yang di-undo. Tombol baru ditambahkan di UI.
#
# --- FITUR SEBELUMNYA ---
# - Pratinjau & Edit OCR
# - Perbaikan Bug & Peningkatan Logika Teks
# - Light & Dark Mode
# - Project Management
#
# --- INSTRUKSI INSTALASI ---
...
#    pip install PyQt5 numpy opencv-python pytesseract requests Pillow easyocr torch torchvision torchaudio
#
...
# --- KONFIGURASI PENTING ---
DEEPL_API_KEY = "2107028d-26f0-4604-a625-80d62f226311:fx" # GANTI DENGAN API KEY ANDA
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v3.7")
    ...
    def setup_right_panel(self, right_panel):
        ...
        ## NEW: Layout untuk tombol Undo/Redo
    ...
    def correct_ocr_errors(self, text):
        correction_map = {'5': 'S', '8': 'B', '0': 'O', '1': 'I', '6': 'G', '4': 'A'}
        corrected_text = list(text)
        for i, char in enumerate(corrected_text):
            if char in correction_map:
                corrected_text[i] = correction_map[char]
    ...
    def run_ocr_and_translate(self, image_to_process, typesetting_rect, polygon=None):
        try:
            ...
            if self.review_checkbox.isChecked():
                dialog = ReviewDialog(processed_text, self)
                if dialog.exec_() == QDialog.Accepted:
                    processed_text = dialog.get_text()
                else:
                    self.progress_bar.setVisible(False)
            ...
            translated_text = self.translate_text(processed_text, target_lang)
            ...
            ## NEW: Hapus redo stack saat ada aksi baru
    ...
    def translate_text(self, text, target_lang):
        if not text.strip(): return "[No text extracted to translate]"
        if not DEEPL_API_KEY or "GANTI_DENGAN_API_KEY_ANDA" in DEEPL_API_KEY:
    ...
    def load_image_at_index(self, index):
        if 0 <= index < len(self.image_files):
            ...
            try:
                ...
                self.redo_stack.clear()
                ## FIX: Jangan reset zoom saat ganti gambar
    ...
    def reset_view_to_original(self):
        if self.original_pixmap:
            ...
            self.redo_stack.clear()
            self.typeset_pixmap = self.original_pixmap.copy()
            self.zoom_factor = 1.0 # Reset zoom hanya di sini
            self.image_label.clear_selection()
            self.update_display()
            self.update_undo_redo_buttons_state()
    ...
    ## NEW: Update state untuk kedua tombol
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v3.7 - {os.path.basename(self.current_project_path)}")
    ...
    def show_about_dialog(self):
        QMessageBox.about(self, "About Manga OCR & Typeset Tool",
            "<b>Manga OCR & Typeset Tool v3.7</b><br><br>"
            "This tool was created to streamline the process of translating manga.<br><br>"
            "Powered by Python, PyQt5, EasyOCR, and Tesseract.<br>"
...
if __name__ == "__main__":
    ...
    if not DEEPL_API_KEY or "GANTI_DENGAN_API_KEY_ANDA" in DEEPL_API_KEY:
        QMessageBox.warning(None, "API Key Missing", "Please provide your valid DeepL API key in the script to enable translation.")
    window = MangaOCRApp()
    window.show()
    sys.exit(app.exec_())
