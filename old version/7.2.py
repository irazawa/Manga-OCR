# Manga OCR & Typeset Tool v7.2
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v7.2 (Perbaikan Bug Kritis) ---
# 1. Perbaikan Tipe Argumen Rendering: Memperbaiki `TypeError` fatal yang
#    terjadi saat menggambar ulang area yang sudah di-typeset. Masalah ini
#    disebabkan oleh penggunaan objek `ImageQt` di mana `QImage` diharapkan.
#    Konversi eksplisit ke `QImage` telah diterapkan untuk menyelesaikan crash.
# 2. Peningkatan Stabilitas Inisialisasi: Menambahkan pemeriksaan tambahan untuk
#    memastikan instance Manga-OCR benar-benar dibuat sebelum digunakan,
#    mencegah error "not initialized" jika terjadi kegagalan saat startup.
#
# --- INSTRUKSI INSTALASI ---
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v7.2")
    ...
    def draw_single_area(self, painter, area):
        ...
        # BARU v7.2: Konversi dari PIL.ImageQt ke QImage sebelum menggambar
        q_image = QImage(ImageQt(inpainted_pil))
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v7.2 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start(); return True
        except Exception as e:
    ...
    def load_project(self):
        ...
        try:
            ...
            self.current_project_path = filePath; self.setWindowTitle(f"Manga OCR & Typeset Tool v7.2 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start()
