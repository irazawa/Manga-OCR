# Manga OCR & Typeset Tool v7.1
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v7.1 (Perbaikan Bug) ---
# 1. Inisialisasi OCR yang Tangguh: Inisialisasi setiap engine OCR (Manga-OCR, EasyOCR)
#    ditangani secara terpisah. Kegagalan pada satu engine tidak akan
#    menghentikan inisialisasi engine lainnya.
# 2. Perbaikan Rendering Gambar: Memperbaiki `TypeError` yang terjadi saat
#    menggambar ulang area yang sudah di-typeset dengan menggunakan konversi
#    gambar yang benar (PIL ke QImage).
# 3. Klarifikasi Teks Vertikal: Menambahkan dokumentasi untuk memperjelas bahwa
#    teks vertikal dari hasil OCR secara otomatis diubah menjadi satu baris
#    horizontal untuk proses terjemahan.
# 4. Logika Typesetting Vertikal yang Lebih Jelas: Mengganti nama fungsi dan
#    menambahkan komentar untuk menjelaskan bahwa typesetting vertikal
#    dicapai dengan merotasi blok teks, yang merupakan pendekatan umum.
#
# --- INSTRUKSI INSTALASI ---
...
from PIL.ImageQt import ImageQt # BARU v7.1: Import eksplisit untuk konversi gambar
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v7.1")
    ...
    def initialize_ocr_engines(self):
        """Inisialisasi engine OCR yang diperlukan saat startup dengan penanganan error terpisah."""
    ...
    def clean_and_join_text(self, raw_text):
        """
        Membersihkan output OCR dengan membaginya menjadi kata-kata dan menggabungkannya dengan spasi tunggal.
        Ini secara efektif mengubah teks multi-baris (misalnya, dari OCR vertikal)
        menjadi satu baris horizontal untuk terjemahan.
        Contoh: "a\nb\nc" menjadi "a b c".
        """
    ...
    def correct_text_with_gemini(self, text_to_correct):
        ...
        if not self.check_and_increment_usage():
            QMessageBox.warning(self, "API Limit Reached", "Gemini API limit has been reached. Proceeding without AI correction for this request.")
            return None
        try:
    ...
    def on_ocr_complete(self, ocr_text, context_data):
        ...
        if self.review_checkbox.isChecked():
            ...
            if dialog.exec_():
                self.start_translate_worker(dialog.get_text(), context_data)
            else:
    ...
    def translate_text(self, text, target_lang):
        ...
        if not DEEPL_API_KEY or "YOUR_" in DEEPL_API_KEY: return "[DEEPL API KEY NOT CONFIGURED]"
        try:
    ...
    def check_and_increment_usage(self):
        ...
        if self.usage_data['daily_count'] >= GEMINI_RPD_LIMIT:
            print("Daily API limit reached."); self.check_limits_and_update_ui(); return False
        ...
        if self.usage_data['minute_count'] >= GEMINI_RPM_LIMIT:
            print(f"Per-minute API limit reached. Try again in {60 - time_since_last_call:.0f} seconds."); self.check_limits_and_update_ui(); return False
    ...
    def load_folder(self):
        ...
        if dir_path:
            ...
            for fname in sorted(os.listdir(dir_path)):
                if fname.lower().endswith(supported_formats): self.image_files.append(os.path.join(dir_path, fname))
    ...
    def on_file_selected(self, current_item, previous_item):
        ...
        if row != self.current_image_index: self.load_image_at_index(row)
    ...
    def draw_rotated_vertical_text(self, painter, rect, text, font, color):
        """
        Mensimulasikan teks vertikal dengan memutar seluruh blok teks sebesar 90 derajat.
        Ini adalah metode sederhana yang cocok untuk banyak gaya manga.
        """
    ...
    def save_image(self):
        ...
        original_filename = os.path.basename(self.image_files[self.current_image_index]); name, _ = os.path.splitext(original_filename)
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v7.1 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start(); return True
        except Exception as e:
    ...
    def load_project(self):
        ...
        try:
            ...
            self.current_project_path = filePath; self.setWindowTitle(f"Manga OCR & Typeset Tool v7.1 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start()
...
if __name__ == "__main__":
    ...
    try:
        ...
        QMessageBox.warning(None, "Tesseract Not Found", "Tesseract was not found in the default path (C:\\Program Files\\Tesseract-OCR\\tesseract.exe).\nPlease install it or correct the path in the script. The 'Tesseract' engine will be disabled.")
    if not MangaOcr:
