# Manga OCR & Typeset Tool v5.0
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v5.0 ---
# 1. Pembaruan Model AI: Mengganti model ke 'gemini-pro' untuk performa yang lebih baik.
# 2. Pelacakan & Pembatasan Penggunaan API Lanjutan:
#    - Melacak panggilan API per menit (RPM) dan per hari (RPD).
#    - Secara otomatis menonaktifkan fitur AI jika batas (15 RPM / 1000 RPD) tercapai.
#    - Menyimpan data penggunaan (termasuk timestamp panggilan terakhir) secara lokal
#      untuk menjaga konsistensi antar sesi.
# 3. Informasi Batasan Real-time: Menu "Help" sekarang menampilkan informasi
#    detail tentang batasan RPM dan RPD serta status penggunaan saat ini.
#
# --- FITUR SEBELUMNYA ---
# - Kontrol Penggunaan AI (Checkbox)
# - Pelacakan Penggunaan Harian Dasar
...
# --- DIUBAH v5.0: Konfigurasi Batasan API ---
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v5.0")
        ...
        # --- DIUBAH v5.0: Struktur Data Pelacakan Penggunaan yang Diperbarui ---
        self.usage_file_path = os.path.join(os.path.expanduser("~"), "manga_ocr_usage_v5.dat")
        self.usage_data = {
            'date': str(date.today()),
            'daily_count': 0,
            'minute_count': 0,
        ...
        # --- PERBAIKAN REKURSI v5.1 ---
        self.load_usage_data()
        self.check_limits_and_update_ui() # Panggil setelah memuat untuk mengatur UI awal
    ...
    def setup_right_panel(self, right_panel):
        ...
        # --- BARU v5.0: Tambahkan tooltip untuk menunjukkan status limit ---
    ...
    def correct_text_with_gemini(self, text_to_correct):
        ...
        # --- DIUBAH v5.0: Panggil fungsi pelacakan sebelum memanggil API ---
        if not self.check_and_increment_usage():
            # Jika limit tercapai, fungsi check_and_increment_usage akan menampilkan pesan
            # dan menonaktifkan checkbox. Cukup kembalikan teks asli.
            return text_to_correct

        try:
            # --- DIUBAH v5.0: Menggunakan model 'gemini-pro' ---
            ...
            # --- BARU v5.0: Jika ada error API, kita tidak ingin menghitungnya sebagai penggunaan yang berhasil ---
            # Namun, untuk kesederhanaan, kita biarkan saja hitungannya bertambah.
            # Logika yang lebih kompleks bisa mengembalikan hitungan jika terjadi error.
            return text_to_correct
    ...
    def run_ocr_and_translate(self, image_to_process, typesetting_rect, polygon=None):
        try:
            ...
            text_for_translation = processed_text # Default text
    ...
    def load_usage_data(self):
        """Hanya memuat data penggunaan dari file dan me-resetnya setiap hari."""
        try:
            if os.path.exists(self.usage_file_path):
                ...
                if self.usage_data.get('date') != str(date.today()):
                    print("New day detected. Resetting API usage stats.")
                    self.usage_data = {
                        'date': str(date.today()),
                        'daily_count': 0,
                        'minute_count': 0,
    ...
    def check_and_increment_usage(self):
        self.load_usage_data() # Selalu dapatkan data terbaru sebelum memeriksa
        ...
        if time_since_last_call > 60:
            # Jika lebih dari satu menit telah berlalu, reset hitungan per menit
        ...
        # Jika semua limit OK, tingkatkan hitungan
    ...
    def check_limits_and_update_ui(self):
        """Memeriksa limit berdasarkan data yang ada dan menonaktifkan checkbox jika perlu."""
        ...
        if daily_limit_reached or minute_limit_reached:
            ...
            if daily_limit_reached:
                self.ai_correction_checkbox.setToolTip("AI Correction disabled: Daily API limit reached.")
                 ...
                 self.ai_correction_checkbox.setToolTip("AI Correction disabled: Per-minute API limit reached.")
        else:
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v5.0 - {os.path.basename(self.current_project_path)}")
    ...
    # --- DIUBAH v5.0: Dialog "About" yang diperbarui dengan informasi limit ---
    def show_about_dialog(self):
        self.load_usage_data() # Pastikan data terbaru
        
        daily_count = self.usage_data.get('daily_count', 0)
        
        # Cek hitungan per menit saat ini
        minute_count = self.usage_data.get('minute_count', 0)
        time_since_last_call = time.time() - self.usage_data.get('last_call_timestamp', 0)
        if time_since_last_call > 60:
            current_rpm = 0 # Reset jika sudah lebih dari semenit
        else:
            current_rpm = minute_count

        about_text = (
            f"<b>Manga OCR & Typeset Tool v5.0</b><br><br>"
            ...
            f"---<br><br>"
            f"<b>Gemini API Usage (gemini-pro):</b><br>"
