...
# 1. File Konfigurasi (config.ini): API Key dan path Tesseract sekarang dikelola
#    di luar skrip dalam file `config.ini`. Ini meningkatkan keamanan dan
#    memudahkan pembaruan.
# 2. Pembuatan Config Otomatis: Jika `config.ini` tidak ditemukan saat aplikasi
#    pertama kali dijalankan, file tersebut akan dibuat secara otomatis dengan
#    nilai placeholder, memandu pengguna untuk melakukan setup.
#
# --- FITUR BARU DI v7.7 (Peningkatan PDF & UX) ---
# 1. Navigasi Scroll PDF: Pengguna dapat menggunakan mouse wheel (scroll) untuk
#    berpindah antar halaman saat melihat dokumen PDF.
# 2. Filter Ekspor PNG: Fitur "Export to PDF" sekarang hanya akan menyertakan
#    file .png.
#
# --- INSTRUKSI INSTALASI ---
...
#    pip install PyMuPDF
#
# 3. Instal Tesseract OCR Engine dari situs resminya.
#
# 4. Jalankan skrip ini sekali untuk membuat file `config.ini`.
# 5. Buka `config.ini` dan masukkan API key Anda serta verifikasi Tesseract path.
