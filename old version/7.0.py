# Manga OCR & Typeset Tool v7.0
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v7.0 ---
# 1. Integrasi Manga-OCR: Menambahkan 'manga-ocr' sebagai engine OCR baru,
#    dioptimalkan untuk teks manga vertikal dan horizontal.
# 2. Kontrol Orientasi Teks: Dropdown baru untuk memilih orientasi teks
#    (Auto, Horizontal, Vertikal) secara manual, meningkatkan akurasi
#    Tesseract dan EasyOCR.
# 3. Perbaikan Alur "Review Before Translate": Proses dipecah menjadi dua
#    tahap (OCR -> Review -> Translate) menggunakan sinyal antar-thread
#    untuk memastikan dialog review muncul dengan benar di main thread tanpa
#    membekukan GUI.
# 4. Perbaikan Mode Batch: Proses batch dibuat lebih tangguh dengan error
#    handling per-gelembung teks. Kegagalan pada satu area tidak akan
#    menghentikan seluruh proses.
# 5. Refactoring Worker Threads: Logika worker dipisahkan menjadi OCRWorker
#    dan TranslateWorker untuk alur kerja yang lebih bersih dan modular.
#
# --- INSTRUKSI INSTALASI ---
...
#
# 3. Instal Tesseract OCR Engine dari situs resminya dan pastikan path-nya benar.
#
...
try:
    # Ganti path ini jika Tesseract diinstal di lokasi lain
...
# --- BARU v7.0: Worker terpisah untuk OCR dan Translate ---

class WorkerSignals(QObject):
    """Sinyal umum untuk komunikasi dari worker thread ke main thread."""
    finished = pyqtSignal()
    error = pyqtSignal(str)
    progress = pyqtSignal(int, str)

class OCRWorkerSignals(WorkerSignals):
    """Sinyal spesifik untuk OCR worker."""
    ocr_complete = pyqtSignal(str, object) # Mengirimkan teks hasil OCR dan data konteks
    batch_result_ready = pyqtSignal(list) # Mengirimkan list of TypesetArea objects

class TranslateWorkerSignals(WorkerSignals):
    """Sinyal spesifik untuk Translation worker."""
    translation_complete = pyqtSignal(str, object) # Mengirimkan teks terjemahan dan data konteks

class OCRWorker(QObject):
    """Worker yang hanya menjalankan tugas OCR di thread terpisah."""
    ...
    def run_batch_processing(self):
        """Proses OCR, AI correct, dan translate untuk seluruh halaman dalam mode batch."""
            ...
            try:
                ...
                if cached_result:
                    ...
                    if not text_for_translation:
                        translated_text = "" # Skip translation for empty text
                    else:
                ...
                if translated_text: # Only add area if there is text
    ...
    def perform_ocr(self, image_to_process):
        ...
        if ocr_engine == "Manga-OCR":
            ...
            # PSM 5 for vertical, 6 for horizontal block
...
class TranslateWorker(QObject):
    """Worker yang hanya menjalankan tugas terjemahan dan AI correction."""
...
class ReviewDialog(QDialog):
    """Dialog untuk mereview dan mengedit teks hasil OCR."""
...
class SelectableImageLabel(QLabel):
    """Label gambar yang mendukung seleksi area."""
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v7.0")
    ...
    def setup_right_panel(self, right_panel):
        ...
        if MangaOcr:
            ocr_engines.insert(0, "Manga-OCR")
        ...
        self.on_ocr_engine_changed(self.ocr_engine_combo.currentText())
        ...
        self.batch_process_button = QPushButton("Batch Process Current Image")
    ...
    def initialize_ocr_engines(self):
        """Inisialisasi engine OCR yang diperlukan saat startup."""
        QApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            # Initialize EasyOCR
            self.statusBar().showMessage("Initializing EasyOCR...")
            lang_list = ['ja', 'en', 'ch_sim', 'ko']
            self.easyocr_reader = easyocr.Reader(lang_list, gpu=True)
            ...
            QMessageBox.critical(self, "OCR Engine Error", f"Could not initialize an OCR engine.\nError: {e}")
        finally:
    ...
    def on_ocr_engine_changed(self, engine):
        ...
        self.lang_combo.setEnabled(is_tesseract)
        
        is_manga_ocr = (engine == "Manga-OCR")
        # Manga-OCR and EasyOCR handle orientation internally or via parameter
        # Tesseract benefits most from manual orientation override
        self.orientation_combo.setEnabled(not is_manga_ocr)
        ...
        if orientation_hint == "Auto-Detect":
            try:
                ...
                if coords is not None:
                    ...
                    angle = rect[-1]
                    if w < h and angle < -45: # Portrait-like
                        angle = -(90 + angle)
                    elif w > h and angle > 45: # Landscape-like
                         angle = 90 - angle
                    else:
                        angle = -angle
            except cv2.error:
            ...
            # Check if it's already mostly vertical, if not, rotate 90 deg
            if w > h:
                angle = 90
    ...
    def on_ocr_complete(self, ocr_text, context_data):
        """Handles the result from the OCR worker."""
        ...
        if self.review_checkbox.isChecked():
            ...
            if dialog.exec_():
                reviewed_text = dialog.get_text()
                self.start_translate_worker(reviewed_text, context_data)
            else: # User cancelled
                self.progress_bar.setVisible(False)
                self.image_label.clear_selection()
        else:
            self.start_translate_worker(ocr_text, context_data)

    def start_translate_worker(self, text_to_process, context_data):
        """Starts the translation worker thread."""
        if self.worker_thread and self.worker_thread.isRunning():
            QMessageBox.warning(self, "Busy", "A process is already running. Please wait.")
            return
        ...
        self.progress_bar.setVisible(True)
        self.progress_bar.setValue(0)
        self.progress_bar.setFormat("Starting Translation...")
    ...
    def on_translation_complete(self, translated_text, context_data):
        """Handles the final result from the translation worker."""
        typeset_rect = context_data.get('typeset_rect')
        polygon = context_data.get('polygon')
    ...
    def start_batch_processing(self):
        ...
        self.worker_thread = QThread()
        # The batch worker is a special case of OCRWorker that handles its own translation loop
    ...
    def on_batch_result_ready(self, new_areas):
        ...
        if not new_areas:
            self.statusBar().showMessage("Batch processing finished, but no text areas were added.", 5000)
            return
    ...
    def translate_text(self, text, target_lang):
        ...
        if not DEEPL_API_KEY or "YOUR_" in DEEPL_API_KEY:
    ...
    def draw_single_area(self, painter, area):
        # Use inpainting for a cleaner background fill
        ...
        # Draw the inpainted background section onto the painter
        source_rect = QRect(area.rect.topLeft(), area.rect.size())
        painter.drawImage(area.rect.topLeft(), inpainted_pil.toqimage(), source_rect)
    ...
    def draw_text_with_options(self, painter, rect, text, font, color, is_vertical=False):
        ...
        while low <= high:
            mid = (low + high) // 2
            current_font.setPointSizeF(mid)
        ...
        current_font.setPointSizeF(best_size)
        painter.setFont(current_font)
    ...
    def draw_vertical_text(self, painter, rect, text, font, color):
        """Draws text vertically, character by character, from top to bottom, right to left."""
        painter.setFont(font)
        painter.setPen(color)
        
        # Simple implementation using rotation
        painter.save()
        center = rect.center()
        ...
        rotated_rect = QRect(0, 0, rect.height(), rect.width())
        rotated_rect.moveCenter(rect.center())
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v7.0 - {os.path.basename(self.current_project_path)}")
    ...
    def load_project(self):
        ...
        try:
            ...
            if index_to_load != -1:
                # This clears typeset_areas and redo_stack, so we must load them again after
    ...
    def show_about_dialog(self):
        ...
        about_text = (
            f"<b>Manga OCR & Typeset Tool v7.0</b><br><br>"
            f"This tool was created to streamline the process of translating manga.<br><br>"
            f"Powered by Python, PyQt5, Manga-OCR, EasyOCR, Tesseract, and Gemini API.<br>"
...
if __name__ == "__main__":
    ...
    try:
        ...
        QMessageBox.warning(None, "Tesseract Not Found", 
                            "Tesseract was not found in the default path (C:\\Program Files\\Tesseract-OCR\\tesseract.exe).\n"
                            "Please install it or correct the path in the script. The 'Tesseract' engine will be disabled.")

    # Check for Manga-OCR
    if not MangaOcr:
         QMessageBox.warning(None, "Manga-OCR Not Found", 
                            "The 'manga-ocr' library is not installed. Please run 'pip install manga-ocr'.\n"
