# Manga OCR & Typeset Tool v7.4
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v7.4 (Perbaikan Stabilitas & Threading) ---
# 1. Perbaikan Alur Kerja Non-Review: Memperbaiki race condition yang menyebabkan
...
# 2. Penanganan Limit API yang Aman untuk Thread: Memperbaiki crash dan freeze
#    saat limit API tercapai. Peringatan dan timer sekarang dikelola
#    sepenuhnya di main thread melalui sinyal, mencegah error threading.
# 3. Sinyal API Limit Baru: Menambahkan sinyal khusus `api_limit_reached` untuk
#    berkomunikasi dari worker ke main thread, memastikan semua dialog dan
#    update UI aman.
#
# --- INSTRUKSI INSTALASI ---
...
class TranslateWorkerSignals(WorkerSignals):
    ...
    api_limit_reached = pyqtSignal() # BARU v7.4
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v7.4")
    ...
    def correct_text_with_gemini(self, text_to_correct):
        ...
        if not self.check_and_increment_usage():
            # BARU v7.4: Jangan tampilkan dialog di sini, kembalikan None
            return None
        try:
    ...
    def on_ocr_complete(self, ocr_text, context_data):
        ...
        if self.review_checkbox.isChecked():
            ...
            if dialog.exec_():
                # PERBAIKAN v7.4: Gunakan QTimer.singleShot untuk menunda eksekusi
    ...
    def start_translate_worker(self, text_to_process, context_data):
        ...
        self.current_worker.signals.error.connect(self.on_worker_error)
        self.current_worker.signals.finished.connect(self.on_worker_finished)
        self.current_worker.signals.api_limit_reached.connect(self.on_api_limit_reached) # BARU v7.4
        self.worker_thread.started.connect(self.current_worker.run)
        self.worker_thread.start()
    ...
    def on_api_limit_reached(self):
        """BARU v7.4: Slot aman untuk menangani UI saat limit API tercapai."""
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v7.4 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start(); return True
        except Exception as e:
    ...
    def load_project(self):
        ...
        try:
            ...
            self.current_project_path = filePath; self.setWindowTitle(f"Manga OCR & Typeset Tool v7.4 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start()
