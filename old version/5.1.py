# Manga OCR & Typeset Tool v5.1
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v5.1 ---
# 1. Timer Otomatis untuk Limit API: Menambahkan QTimer yang secara otomatis
#    mengaktifkan kembali fitur AI setelah masa tunggu limit per menit (RPM) selesai.
# 2. Tooltip dengan Hitungan Mundur: Tooltip pada checkbox AI sekarang menampilkan
#    hitungan mundur (dalam detik) kapan fitur akan tersedia kembali.
# 3. Pengaktifan Otomatis: Setelah cooldown selesai, checkbox AI tidak hanya
#    diaktifkan, tetapi juga otomatis dicentang untuk kemudahan penggunaan.
#
# --- FITUR SEBELUMNYA (v5.0) ---
# - Pembaruan Model AI ke 'gemini-2.5-flash-lite'
# - Pelacakan & Pembatasan Penggunaan API (RPM & RPD)
# - Kontrol Penggunaan AI, Integrasi AI, Zoom Tetap & Fungsi Redo
#
# --- INSTRUKSI INSTALASI ---
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v5.1")
        ...
        # --- BARU v5.1: Timer untuk memeriksa limit API secara berkala ---
        self.api_limit_timer = QTimer(self)
        self.api_limit_timer.setInterval(1000)  # Periksa setiap 1 detik
    ...
    # --- DIUBAH v5.1: Logika untuk memulai/menghentikan timer ---
    def check_limits_and_update_ui(self):
        """
        Memeriksa limit berdasarkan data yang ada, menonaktifkan checkbox jika perlu,
        dan mengelola state dari QTimer.
        """
    ...
    # --- BARU v5.1: Fungsi yang dipanggil oleh QTimer ---
    def periodic_limit_check(self):
        """
        Dipanggil oleh QTimer setiap detik untuk memeriksa apakah cooldown limit API telah berlalu.
        """
        if not self.ai_correction_checkbox.isEnabled():
            ...
            if time_since_last_call > 60:
                ...
                self.ai_correction_checkbox.setEnabled(True)
                self.ai_correction_checkbox.setChecked(True) # Aktifkan dan centang otomatis
                self.ai_correction_checkbox.setToolTip("Enable/Disable AI-powered text correction using Gemini API.")
                self.api_limit_timer.stop()
            else:
                # Perbarui hitungan mundur di tooltip
                remaining_time = 61 - int(time_since_last_call)
                self.ai_correction_checkbox.setToolTip(f"AI Correction disabled: Per-minute limit reached. Available in {remaining_time}s.")
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v5.1 - {os.path.basename(self.current_project_path)}")
    ...
    def show_about_dialog(self):
        ...
        about_text = (
            f"<b>Manga OCR & Typeset Tool v5.1</b><br><br>"
