# Manga OCR & Typeset Tool v7.7
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v7.7 (Peningkatan PDF & UX) ---
# 1. Navigasi Scroll PDF: Pengguna sekarang dapat menggunakan mouse wheel (scroll)
#    untuk berpindah antar halaman saat melihat dokumen PDF, memberikan
#    pengalaman navigasi yang lebih cepat dan intuitif.
# 2. Filter Ekspor PNG: Fitur "Export to PDF" sekarang secara spesifik hanya
#    akan menyertakan file .png, memastikan output yang lebih bersih dan terprediksi.
#
...
#    langsung di jendela utama.
# 3. Fungsionalitas Penuh pada PDF: Semua alat OCR dan typesetting berfungsi pada
#    halaman PDF yang sedang ditampilkan.
#
# --- INSTRUKSI INSTALASI ---
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v7.7")
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v7.7 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start(); return True
        except Exception as e:
    ...
    def load_project(self):
        ...
        try:
            ...
            self.current_project_path = filePath; self.setWindowTitle(f"Manga OCR & Typeset Tool v7.7 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start()
    ...
    # --- BARU v7.7 --- Handle mouse wheel events for PDF navigation
    def wheelEvent(self, event):
        # Hanya aktifkan scroll jika PDF sedang dibuka dan tidak ada proses yang berjalan
        if self.pdf_document and not (self.worker_thread and self.worker_thread.isRunning()):
            # Scroll ke bawah (maju)
            if event.angleDelta().y() < 0:
                self.load_next_image()
            # Scroll ke atas (mundur)
            elif event.angleDelta().y() > 0:
                self.load_prev_image()
        
        # Panggil event handler default untuk fungsionalitas scroll lainnya
