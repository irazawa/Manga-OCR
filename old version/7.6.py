# Manga OCR & Typeset Tool v7.6
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v7.6 (Integrasi PDF) ---
# 1. Ekspor ke PDF: Menambahkan opsi "File > Export Folder to PDF..." untuk
#    mengonversi semua gambar di folder saat ini menjadi satu file PDF.
#    File diurutkan secara numerik (natural sort) untuk memastikan urutan yang benar.
# 2. Penampil PDF Internal: Aplikasi sekarang dapat membuka dan menampilkan file .pdf
#    langsung di jendela utama. Pengguna dapat menavigasi halaman PDF
#    menggunakan tombol "Next" dan "Previous".
# 3. Fungsionalitas Penuh pada PDF: Semua alat OCR, seleksi, dan typesetting
#    berfungsi pada halaman PDF yang sedang ditampilkan, sama seperti pada gambar biasa.
#
# --- FITUR BARU DI v7.5 (Perbaikan Stabilitas Kritis) ---
# 1. Perbaikan Threading Limit API: Memperbaiki crash "QObject::startTimer: Timers cannot be started from another thread"
#    yang terjadi saat limit API tercapai. Logika UI (memulai timer, menampilkan dialog)
#    sekarang dipisahkan sepenuhnya dari logika pengecekan limit, memastikan semua
#    operasi UI berjalan aman di main thread.
# 2. Alur Kerja Non-Review yang Lebih Mulus: Memperbaiki race condition yang menyebabkan
#    error "A process is already running" saat checkbox "Review Before Translate"
#    tidak dicentang. Transisi dari OCR ke Translate sekarang dijadwalkan
#    dengan benar, memberikan waktu bagi thread sebelumnya untuk selesai.
#
# --- INSTRUKSI INSTALASI ---
...
#    pip install PyMuPDF # --- BARU v7.6 --- Diperlukan untuk fitur PDF
#
...
import hashlib
import re # --- BARU v7.6 ---
import fitz # --- BARU v7.6 --- (from PyMuPDF)
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v7.6")
    ...
    def load_folder(self):
        ...
        if dir_path:
            # --- BARU v7.6 --- Reset PDF state when loading a new folder
            ...
            # --- BARU v7.6 --- Add PDF to supported formats
            supported_formats = ('.png', '.jpg', '.jpeg', '.bmp', '.webp', '.pdf')
            
            # --- BARU v7.6 --- Use natural sort for file listing
    ...
    # --- BARU v7.6 --- Renamed and updated to handle both images and PDFs
    def load_item_at_index(self, index):
        if 0 <= index < len(self.image_files):
            self.current_image_index = index
            file_path = self.image_files[index]
            
            # Reset PDF state if we are not loading a PDF
    ...
    # --- BARU v7.6 --- Logic for loading standard image files
    ...
    # --- BARU v7.6 --- Logic for loading a PDF file
    ...
    # --- BARU v7.6 --- Logic for rendering a specific PDF page
    def load_pdf_page(self, page_number):
        ...
        # Render page to a pixmap with good resolution
    ...
    def update_nav_buttons(self):
        # --- BARU v7.6 --- Update button states based on whether a PDF or image is loaded
        if self.pdf_document:
    ...
    def save_image(self):
        ...
        # --- BARU v7.6 --- Suggest filename based on PDF page or image name
        if self.pdf_document:
    ...
    def save_project(self):
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v7.6 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start(); return True
        except Exception as e:
    ...
    def load_project(self):
        ...
        try:
            ...
            if index_to_load != -1:
                # This will call load_item_at_index, which handles both images and PDFs
            ...
            self.current_project_path = filePath; self.setWindowTitle(f"Manga OCR & Typeset Tool v7.6 - {os.path.basename(self.current_project_path)}"); self.autosave_timer.start()
    ...
    def check_cache(self, key):
        ...
        # --- BARU v7.6 --- Add page number to cache filename for PDFs
        if self.pdf_document:
    ...
    # --- BARU v7.6 --- New function to export images to a single PDF
    def export_to_pdf(self):
        if not self.project_dir:
            QMessageBox.warning(self, "No Folder Loaded", "Please load a folder containing images first.")
            return

        image_files_to_export = [f for f in self.image_files if not f.lower().endswith('.pdf')]

        if not image_files_to_export:
            QMessageBox.warning(self, "No Images Found", "The current folder contains no image files to export.")
            return
        ...
        # Use natural sort to order files correctly (e.g., 1.png, 2.png, 10.png)
        def natural_sort_key(s):
