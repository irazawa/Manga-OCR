# Manga OCR & Typeset Tool v6.0
# Ditingkatkan oleh Gemini
#
# --- FITUR BARU DI v6.0 ---
# 1. Threading untuk GUI Responsif: Semua proses berat (OCR, Translate, Batch)
#    dipindahkan ke background thread (QThread), mencegah GUI freeze.
# 2. Mode Batch Otomatis: Tombol "Batch Process All Images" untuk mendeteksi,
#    menerjemahkan, dan men-typeset semua teks di semua gambar secara otomatis.
# 3. Optimasi OCR Pipeline:
#    - Rotasi Otomatis: Area teks dideteksi dan dirotasi ke posisi horizontal
#      sebelum OCR untuk akurasi yang lebih tinggi.
#    - Adaptive Thresholding: Meningkatkan kontras teks secara otomatis.
# 4. Caching Hasil: Hasil OCR & terjemahan disimpan di folder .cache untuk
#    mempercepat pemrosesan ulang dan menghemat panggilan API.
# 5. Typesetting Vertikal: Opsi baru untuk merender teks terjemahan secara
#    vertikal, cocok untuk manga.
# 6. Auto-Save: Proyek disimpan secara otomatis setiap 5 menit untuk
#    mencegah kehilangan data.
# 7. Penyimpanan State Undo/Redo: Tumpukan undo/redo sekarang disimpan
#    dalam file proyek.
#
# --- INSTRUKSI INSTALASI ---
...
from PyQt5.QtGui import (
    QPixmap, QPainter, QPen, QColor, QFont, QKeySequence, QPolygon, QPainterPath, QPolygonF, QTransform
)
from PyQt5.QtCore import Qt, QRect, QPoint, QSize, pyqtSignal, QTimer, QThread, QObject
...
# --- BARU v6.0: Worker untuk proses di background thread ---
class OCRWorkerSignals(QObject):
    """Sinyal untuk komunikasi dari worker thread ke main thread."""
    finished = pyqtSignal()
    error = pyqtSignal(str)
    progress = pyqtSignal(int, str)
    result_ready = pyqtSignal(object) # Mengirimkan TypesetArea object
    batch_result_ready = pyqtSignal(list) # Mengirimkan list of TypesetArea objects

class OCRWorker(QObject):
    """Worker yang menjalankan tugas OCR dan terjemahan di thread terpisah."""
    def __init__(self, main_app, image_to_process, settings, typeset_rect=None, polygon=None, is_batch=False):
        super().__init__()
        self.main_app = main_app
        self.image_to_process = image_to_process
        self.settings = settings
        self.typeset_rect = typeset_rect
    ...
    def run_single_ocr(self):
        """Proses OCR untuk satu area."""
        self.signals.progress.emit(10, "Preprocessing image...")
        preprocessed_image, _ = self.main_app.preprocess_for_ocr(self.image_to_process)

        # Caching
        cache_key = self.main_app.get_cache_key(preprocessed_image)
        cached_result = self.main_app.check_cache(cache_key)
        if cached_result:
            self.signals.progress.emit(80, "Result from cache...")
            translated_text = cached_result['translated_text']
        else:
            # Lanjutkan proses jika tidak ada di cache
            self.signals.progress.emit(30, "Running OCR...")
            ...
            if self.settings['use_ai']:
                self.signals.progress.emit(50, "Correcting with AI...")
                corrected_text = self.main_app.correct_text_with_gemini(processed_text)
                if corrected_text is not None:
                    text_for_translation = corrected_text
                else: # API limit reached or error
                    self.signals.error.emit("AI correction skipped due to API limit or error.")

            if self.settings['review_before_translate']:
                # Note: Review dialog needs to be handled in the main thread.
                # This part of the logic is simplified for threading.
                # A more robust solution would involve more complex signal/slot management.
                pass
        ...
        new_area = TypesetArea(self.typeset_rect, translated_text, self.settings['font'], self.settings['color'], self.polygon)
        self.signals.result_ready.emit(new_area)

    def run_batch_processing(self):
        """Proses OCR untuk seluruh halaman dalam mode batch."""
        ...
        # Gunakan EasyOCR untuk deteksi paragraf/gelembung teks
        if not self.main_app.easyocr_reader:
            self.signals.error.emit("EasyOCR not initialized for batch mode.")
            return
        
        results = self.main_app.easyocr_reader.readtext(gray, detail=1, paragraph=True)
        total_bubbles = len(results)
        processed_areas = []

        for i, (bbox, text, conf) in enumerate(results):
            self.signals.progress.emit(int(10 + (i / total_bubbles) * 60), f"Processing bubble {i+1}/{total_bubbles}...")
            
            # Bbox format: [[x_min, y_min], [x_max, y_min], [x_max, y_max], [x_min, y_max]]
            x_min, y_min = map(int, bbox[0])
            x_max, y_max = map(int, bbox[2])
            
            # Buat QRect dari bbox
            typeset_rect = QRect(QPoint(x_min, y_min), QPoint(x_max, y_max))

            # Crop image untuk bubble ini
            ...
            # Preprocess
            preprocessed_image, _ = self.main_app.preprocess_for_ocr(cropped_cv_img)
    ...
    def perform_ocr(self, image_to_process):
        """Melakukan OCR pada gambar yang sudah diproses."""
        ...
        if ocr_engine == "EasyOCR":
            ...
            gray = cv2.cvtColor(image_to_process, cv2.COLOR_BGR2GRAY)
            # Tesseract bekerja lebih baik pada teks horizontal
            # Rotasi sudah dilakukan di preprocess_for_ocr
            lang_map = {"Japanese": "jpn", "English": "eng", "Chinese": "chi_sim", "Korean": "kor"}
            tess_lang = lang_map.get(self.settings['ocr_lang'], "eng")
            custom_config = f'--oem 1 --psm 6 -l {tess_lang}' # PSM 6: Assume a single uniform block of text.
...
class TypesetArea:
    ...
    def font_to_dict(font):
        return {
            'family': font.family(), 'pointSize': font.pointSize(),
...
class MangaOCRApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Manga OCR & Typeset Tool v6.0")
        ...
        # API Usage Tracking
        self.usage_file_path = os.path.join(os.path.expanduser("~"), "manga_ocr_usage_v6.dat")
        self.usage_data = {
            'date': str(date.today()), 'daily_count': 0,
            'minute_count': 0, 'last_call_timestamp': 0
        }
        ...
        self.autosave_timer.setInterval(300000) # 5 menit
    ...
    def setup_menu_bar(self):
        ...
        about_action = QAction('About & API Usage', self)
    ...
    def setup_right_panel(self, right_panel):
        ...
        # EasyOCR advanced options
        easyocr_options_layout = QHBoxLayout()
        easyocr_options_layout.addWidget(QLabel("Contrast Ths:"))
        self.contrast_ths_spinbox = QDoubleSpinBox()
        self.contrast_ths_spinbox.setRange(0.0, 1.0)
        self.contrast_ths_spinbox.setSingleStep(0.05)
        self.contrast_ths_spinbox.setValue(0.15)
        easyocr_options_layout.addWidget(self.contrast_ths_spinbox)
        easyocr_options_layout.addWidget(QLabel("Adjust Contrast:"))
        self.adjust_contrast_spinbox = QDoubleSpinBox()
        self.adjust_contrast_spinbox.setRange(0.0, 1.0)
        self.adjust_contrast_spinbox.setSingleStep(0.05)
        self.adjust_contrast_spinbox.setValue(0.5)
        easyocr_options_layout.addWidget(self.adjust_contrast_spinbox)
        right_panel.addLayout(easyocr_options_layout)

        self.ai_correction_checkbox = QCheckBox("Use AI to Correct Text")
        self.ai_correction_checkbox.setChecked(True)
        self.ai_correction_checkbox.setToolTip("Enable/Disable AI-powered text correction using Gemini API.")
        right_panel.addWidget(self.ai_correction_checkbox)
        ...
        right_panel.addWidget(self.vertical_typeset_checkbox)

        self.style_transfer_button = QPushButton("Load Custom OCR Model (Concept)")
        ...
        self.batch_process_button = QPushButton("Batch Process All Images")
        self.batch_process_button.clicked.connect(self.start_batch_processing)
        right_panel.addWidget(self.batch_process_button)
    ...
    def update_easyocr_reader(self):
        ...
        try:
            ...
            self.statusBar().showMessage("EasyOCR reader initialized successfully.", 3000)
        except Exception as e:
    ...
    def on_ocr_engine_changed(self, engine):
        ...
        self.contrast_ths_spinbox.setEnabled(is_easyocr)
        self.adjust_contrast_spinbox.setEnabled(is_easyocr)
        if is_easyocr and not self.easyocr_reader:
            self.update_easyocr_reader()
    ...
    def correct_text_with_gemini(self, text_to_correct):
        ...
        try:
            ...
            prompt = (
                "Correct the grammar, spelling, and capitalization of the following text from an OCR scan of a manga. "
    ...
    # --- BARU v6.0: Preprocessing Pipeline ---
    def preprocess_for_ocr(self, cv_image):
        """Preprocesses an image for OCR by rotating and applying thresholding."""
        gray = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)
        ...
        # Use minAreaRect on the whole image to find general text orientation
        # A more advanced method would be to find contours first
        coords = cv2.findNonZero(cv2.bitwise_not(gray))
        if coords is None: return cv_image, 0 # Empty image
        ...
        try:
            ...
            if angle < -45:
                ...
                angle = -angle
        except cv2.error: # Can happen on very small or uniform images
            angle = 0

        # Rotate the image to make text horizontal
        center = (w // 2, h // 2)
        ...
        preprocessed_bgr = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)
        return preprocessed_bgr, angle

    def run_ocr_and_translate(self, image_to_process, typesetting_rect, polygon=None):
        """Starts the OCR worker thread for a single area."""
        ...
        # Collect settings for the worker
        settings = {
            ...
            'use_ai': self.ai_correction_checkbox.isChecked(),
            'review_before_translate': self.review_checkbox.isChecked(),
            'font': self.typeset_font,
            'color': self.typeset_color
        }
        
        self.worker_thread = QThread()
        self.ocr_worker = OCRWorker(self, image_to_process, settings, typesetting_rect, polygon)
        self.ocr_worker.moveToThread(self.worker_thread)

        # Connect signals
        self.ocr_worker.signals.result_ready.connect(self.on_ocr_result_ready)
        self.ocr_worker.signals.progress.connect(self.update_progress)
    ...
    def start_batch_processing(self):
        """Starts the OCR worker for batch processing the current image."""
        ...
        self.progress_bar.setVisible(True)
        self.progress_bar.setValue(0)
        self.progress_bar.setFormat("Starting Batch Mode...")

        settings = {
            'ocr_engine': "EasyOCR", # Batch mode relies on EasyOCR for bubble detection
            'target_lang': self.translate_combo.currentText(),
            'use_ai': self.ai_correction_checkbox.isChecked(),
            'font': self.typeset_font,
            'color': self.typeset_color
        ...
        self.worker_thread = QThread()
        self.ocr_worker = OCRWorker(self, self.current_image_pil, settings, is_batch=True)
        self.ocr_worker.moveToThread(self.worker_thread)

        # Connect signals
        self.ocr_worker.signals.batch_result_ready.connect(self.on_batch_result_ready)
        self.ocr_worker.signals.progress.connect(self.update_progress)
        self.ocr_worker.signals.error.connect(self.on_ocr_error)
        self.ocr_worker.signals.finished.connect(self.on_ocr_finished)
        self.worker_thread.started.connect(self.ocr_worker.run)
    ...
    def on_ocr_result_ready(self, new_area):
        """Handles the result from a single OCR process."""
        ...
        # This part is tricky as the original text isn't passed back from the worker easily
    ...
    def on_batch_result_ready(self, new_areas):
        """Handles the result from a batch process."""
        self.typeset_areas.extend(new_areas)
        self.redo_stack.clear()
        self.redraw_all_typeset_areas()
        self.update_undo_redo_buttons_state()
    ...
    def on_ocr_finished(self):
        """Cleans up the thread after the worker is done."""
        self.progress_bar.setVisible(False)
    ...
    def check_and_increment_usage(self):
        ...
        if time_since_last_call > 60:
            self.usage_data['minute_count'] = 0
        
        if self.usage_data['minute_count'] >= GEMINI_RPM_LIMIT:
            print(f"Per-minute API limit reached. Try again in {60 - time_since_last_call:.0f} seconds.")
            self.check_limits_and_update_ui()
            return False

        self.usage_data['daily_count'] += 1
        self.usage_data['minute_count'] += 1
        self.usage_data['last_call_timestamp'] = current_time
        
        self.save_usage_data()
        print(f"Gemini API call count: {self.usage_data['minute_count']}(RPM), {self.usage_data['daily_count']}(RPD)")
        return True
    ...
    def check_limits_and_update_ui(self):
        ...
        if time_since_last_call <= 60:
            if self.usage_data.get('minute_count', 0) >= GEMINI_RPM_LIMIT:
                minute_limit_reached = True

        if daily_limit_reached:
            self.ai_correction_checkbox.setChecked(False)
            self.ai_correction_checkbox.setEnabled(False)
            self.ai_correction_checkbox.setToolTip("AI Correction disabled: Daily API limit reached.")
            if self.api_limit_timer.isActive():
                self.api_limit_timer.stop()
            ...
            if not self.api_limit_timer.isActive():
                self.api_limit_timer.start()
        else:
    ...
    def process_rect_area(self, selection_rect):
        ...
        cropped_cv_img = cv2.cvtColor(np.array(cropped_img), cv2.COLOR_RGB2BGR)
        self.run_ocr_and_translate(cropped_cv_img, unzoomed_rect)
    ...
    def draw_text_with_options(self, painter, rect, text, font, color, is_vertical=False):
        if is_vertical:
            self.draw_vertical_text(painter, rect, text, font, color)
            return
    ...
    def draw_vertical_text(self, painter, rect, text, font, color):
        """Draws text vertically, character by character."""
        painter.setFont(font)
        painter.setPen(color)

        # Simple implementation: rotate the painter
        painter.save()
        center = rect.center()
        ...
        # Swap width and height for the rotated rect
        rotated_rect = QRect(rect.y(), rect.x(), rect.height(), rect.width())
        self.draw_text_with_options(painter, rotated_rect, text, font, color, is_vertical=False) # Reuse horizontal logic
        painter.restore()
    ...
    def save_project(self):
        ...
        project_data = {
            ...
            'redo_stack': self.redo_stack, # BARU v6.0
        ...
        try:
            ...
            self.setWindowTitle(f"Manga OCR & Typeset Tool v6.0 - {os.path.basename(self.current_project_path)}")
            self.autosave_timer.start() # Start autosave timer after first successful save
            return True
        except Exception as e:
    ...
    def load_project(self):
        ...
        try:
            ...
            self.redo_stack = project_data.get('redo_stack', []) # BARU v6.0
            ...
            if index_to_load != -1:
                self.load_image_at_index(index_to_load) # This clears typeset_areas and redo_stack
                # So we must load them again after
                self.typeset_areas = project_data.get('typeset_areas', [])
                self.redo_stack = project_data.get('redo_stack', [])
    ...
    def show_style_transfer_info(self):
        QMessageBox.information(self, "Conceptual Feature: Custom Model",
            "This is a placeholder for a future feature where you could load a custom-trained EasyOCR model file for specific font styles.")
    ...
    def show_about_dialog(self):
        ...
        minute_count = self.usage_data.get('minute_count', 0)
        time_since_last_call = time.time() - self.usage_data.get('last_call_timestamp', 0)
        current_rpm = 0 if time_since_last_call > 60 else minute_count

        about_text = (
            f"<b>Manga OCR & Typeset Tool v6.0</b><br><br>"
            f"This tool was created to streamline the process of translating manga.<br><br>"
            f"Powered by Python, PyQt5, EasyOCR, Tesseract, and Gemini API.<br>"
            f"Enhanced with new features by Gemini.<br><br>"
            f"---<br><br>"
            f"<b>Gemini API Usage (gemini-1.5-flash-latest):</b><br>"
            f" - Usage This Minute: <b>{current_rpm} / {GEMINI_RPM_LIMIT}</b> RPM<br>"
            f" - Usage Today: <b>{daily_count} / {GEMINI_RPD_LIMIT}</b> RPD<br><br>"
            f"<i>The AI feature will be disabled automatically if a limit is reached. "
            f"The RPM count resets 60 seconds after the first call in a new minute, and the RPD count resets daily.</i><br><br>"
            f"Copyright © 2024"
        )
        QMessageBox.about(self, "About & API Usage", about_text)

    # --- BARU v6.0: Caching Functions ---
    def get_cache_key(self, cv_image):
        """Generates a unique key for a given image crop."""
        image_bytes = cv2.imencode('.png', cv_image)[1].tobytes()
        return hashlib.md5(image_bytes).hexdigest()

    def check_cache(self, key):
        """Checks if a result for a key exists in the cache."""
        if not self.cache_dir: return None
        current_image_name = os.path.basename(self.image_files[self.current_image_index])
        cache_file = os.path.join(self.cache_dir, f"{current_image_name}.json")
    ...
    def write_to_cache(self, key, data):
        """Writes a result to the cache file for the current image."""
        ...
        if os.path.exists(cache_file):
            with open(cache_file, 'r') as f:
                try:
                    ...
                    cache_data = {} # Overwrite corrupted file
